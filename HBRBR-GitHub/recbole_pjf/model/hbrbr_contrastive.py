...
# ============================================================
# 消融变体 3: Contrastive Loss —— 用对比学习 (InfoNCE) 替代对抗训练
# 目的: 对抗机制 vs 对比目标的消融对比，回应"compare with contrastive objectives"
# ============================================================
...
class BGNN_Contrastive(nn.Module):
    """GCN 传播 + 对比学习对齐，无对抗模块 —— 用 InfoNCE 替代 GAN"""

    def __init__(self, config=None, temperature=0.07):
        super(BGNN_Contrastive, self).__init__()
        ...
        self.tau = temperature  # InfoNCE 温度系数
    ...
    def _info_nce_loss(self, real_data, gcn_output):
        """双向 InfoNCE 对比损失"""
        real_norm = F.normalize(real_data, dim=1)
        gcn_norm = F.normalize(gcn_output, dim=1)
        sim = torch.mm(real_norm, gcn_norm.T) / self.tau  # (B, B)
        labels = torch.arange(sim.size(0), device=self.device)
        loss_u2v = F.cross_entropy(sim, labels)
        loss_v2u = F.cross_entropy(sim.T, labels)
        return (loss_u2v + loss_v2u) / 2
    ...
    def __layer_inference(self, gcn, optimizer_G, real_batch_num, real_num,
        ...
        for i in range(self.epochs):
            for iter in range(real_batch_num):
                ...
                if is_training:
                    loss_cl = self._info_nce_loss(attr_batch, gcn_output)
    ...
    def contrastive_propagation(self, u_previous_embedding, v_previous_embedding,
...
class HBRBR_Contrastive(GeneralRecommender):
    """消融变体3: GCN 传播 + 对比学习 (InfoNCE)，无对抗训练"""
    input_type = InputType.PAIRWISE

    def __init__(self, config, dataset):
        super(HBRBR_Contrastive, self).__init__(config, dataset)
        ...
        # 可从 config 读取温度系数，默认 0.07
        tau = config['contrastive_temperature'] if 'contrastive_temperature' in config else 0.07
        self.bgnn = BGNN_Contrastive(temperature=tau)
    ...
    def forward(self, geek_sents, job_sents, is_training=True):
        ...
        geek_vec_bgnn, job_vec_bgnn = self.bgnn.contrastive_propagation(
