...
class BGNN(object):
    def __init__(self):
        ...
        self.dis_hidden_dim = 1
        self.device = torch.device("cuda:0" if torch.cuda.is_available() and 0 else "cpu")
        self.batch_size = 100

        self.v_attr_dimensions = 64         # 输入特征的维度
...
# Alias so the framework's get_model('IPJFNN') can load this BGNN-enhanced model
# (get_model lowercases the name -> ipjfnn.py, then looks up the class IPJFNN).
IPJFNN = PJFNN
