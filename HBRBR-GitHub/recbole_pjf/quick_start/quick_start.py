...
def eval_recbole_pjf(model=None, dataset=None, config_file_list=None, config_dict=None, saved=True, model_file = None):
    # configurations initialization
    config = PJFConfig(model=model, dataset=dataset, config_file_list=config_file_list, config_dict=config_dict)
    init_seed(config['seed'], config['reproducibility'])
    # logger initialization
    ...
    # model evaluation
    test_result = trainer.evaluate(test_data, load_best_model=saved, show_progress=config['show_progress'], model_file = model_file)
